...
class OTP:
    ...
    def BuildNVTDict(self, nvt_checksum):
        ...
        oid.Parser(rawOid)
        with open('./currentnvt.json', 'w+') as nvt_dict_file:
            json.dump(oid.familyDict, nvt_dict_file)
        with open('./nvtchecksum.conf', 'w+') as nvt_checksum_file:
    ...
    def ListFamilies(self):
        ...
        if os.path.isfile('./nvtchecksum.conf'):
            with open('./nvtchecksum.conf', 'r') as nvt_checksum_file:
                nvt_checksum_file = nvt_checksum_file.read()
            if nvt_checksum_file == nvt_checksum:
                self.sock.Send("\n")
                test = self.sock.Receive()
                with open('./currentnvt.json', 'r') as nvt_dict_file:
